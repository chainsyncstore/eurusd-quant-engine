...
with patch("quant.live.signal_generator.CapitalClient"), \
...
def setup_generator():
    ...
    with patch("builtins.open"), \
        ...
        gen = SignalGenerator(model_dir=mock_dir, capital=10000)
        gen.client = MagicMock()
        gen.client._cfg.epic = "BTCUSD"
        gen._authenticated = True
        return gen

def test_execution_buy_no_position():
    gen = setup_generator()
    gen.client.get_positions.return_value = [] # No current positions
    ...
    gen.client.place_order.assert_called_once_with(
        epic="BTCUSD",
        direction="BUY",
        size=1.5
    )
...
def test_execution_hold_does_nothing():
    gen = setup_generator()
    gen.client.get_positions.return_value = []
    ...
    gen.client.place_order.assert_not_called()
    gen.client.close_position.assert_not_called()
...
def test_execution_flip_sell_to_buy():
    ...
    gen.client.close_position.assert_called_once_with("DEAL_123")
    # And open new long
    gen.client.place_order.assert_called_once_with(
        epic="BTCUSD",
        direction="BUY",
